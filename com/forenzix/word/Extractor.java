...
public abstract class Extractor {
	 ...
	 * public static List<Replacer> generateReplacers(XWPFDocument doc, Claim claim,
	 * IContext context) {
	 * final List<String> tags = extractTags(doc);
	 * final Map<String, Object> vars = new HashMap<>();
	 * 
	 * final IMendixObject claimCalc = Microflows.get_ClaimCalc(context,
	 * claim).getMendixObject();
	 * final List<Contract> contracts = Core.
	 * createXPathQuery("//Calculators.Contract[Calculators.Contract_Claim = $claim]"
	 * )
	 ...
	 * final Set claimMembers = new
	 * HashSet<>(claim.getMendixObject().getMembers(context).entrySet());
	 * claimMembers.addAll(claimCalc.getMembers(context).entrySet());
	 * 
	 * for (var entry : (Set<Map.Entry<String, IMendixObjectMember<?>>>)
	 * claimMembers) {
	 ...
	 * 
	 * contractCalcs.put(contract.getMendixObject().getId(),
	 * Microflows.get_ContractCalc(context, contract).getMendixObject());
	 * }
	 * 
	 * // Additional custom variable
	 * final Account user =
	 * usermanagement.proxies.microflows.Microflows.get_User_Account(context);
	 ...
	 * if (contractCalcs.containsKey(mobject.getId()) && (mcalc =
	 * contractCalcs.get(mobject.getId())).hasMember(member)) {
	 * mobject = mcalc;
	 * }
	 * 
	 * final Object value;
	 ...
	 * -> {
	 * ((IMendixObject) object).setValue(context, member, value instanceof
	 * IMendixObject ?
	 ...
	 * else {
	 * throw new
	 * IllegalArgumentException("Tag should start and end with double or triple angle brackets (<>)."
			...
			if (contents.charAt(i) == '<' && contents.charAt(i + 1) == '<') {
				for (int j = i + minimum.length() - 1; j < contents.length(); j += 1) {
					if (contents.charAt(j - 1) == '>' && contents.charAt(j) == '>') {
						...
						if (j + 1 < contents.length() && contents.charAt(j + 1) == '>')
							continue;
