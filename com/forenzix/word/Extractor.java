...
public abstract class Extractor {
	...
	// Mendix Specifc Methods
	/**
	 * Convenience function that extracts special tags from a document, then 
	 * generates corresponding replacer objects in one go.
	 * <p>
	 ...
	 * @param claim claim object which contains relevant data to be processed
	 * @return 		list of corresponding replacers
	 */
	// public static List<Replacer> extractSpecialReplacers(XWPFDocument doc, Claim claim) {
	// 	return Replacers.generateReplacers(extractSpecialTags(doc), claim, claim.getContext());
	// }

	/**
	 * Convenience function that extracts tags from a document, then generates 
	 * corresponding replacer objects in one go. 
	 * 
	 * @param doc 	input document with tags to be extracted
	 * @param claim claim object which contains relevant data to be processed
	 * @return 		list of corresponding replacers
	 */
	// public static List<Replacer> extractReplacers(XWPFDocument doc, Claim claim) {
	// 	return Replacers.generateReplacers(extractTags(doc), claim, claim.getContext());
	// }
	 ...
	 * {@code &lt;&lt;i = i + 1;&gt;&gt;&gt;} is not.
	 * <p>Tags can have a format spefication at the end of the tag preceeded by
	 ...
	 * information about how the format spec works, check out {@link Replacers#format( 
	...
	public static List<String> extractTags(XWPFDocument doc) {
		...
		// Extractor states
		final int 
			WAITING = 0,   // indicates we have yet to encounter a tag 
