...
/**
 * An Apache POI based document processor. It is made for the automatic replication
 * of specific portions of an {@link XWPFDocument} object. This class has the potential
...
public final class Preprocessor {
     ...
     * @param times the number of times for which the tagged sections shall be repeated
     */
    public static void repeatSections(XWPFDocument doc, long times) {
        XmlCursor cursor; IBodyElement copyable, element;
        ...
        for (i = 0, start = 0; i < doc.getBodyElements().size(); i += 1) {
            ...
            // ... until a paragraph that contains the tags 'repeat start' or 'repeat end' is found
    ...
    // This function sometimes is used by classes other than this one. Change it sparsely.
    ...
    private static void enumerateParagraph(XWPFParagraph p, int ordinal) {
        ...
        for (int r = 0; r < p.getRuns().size(); r += 1) {
            ...
            // Potential issue
            if (run.text().contains("<<")) permit = true;
            else if (run.text().contains(">>")) permit = false;
            ...
            if (r < p.getRuns().size() - 1 && run.text().endsWith("C") && p.getRuns().get(r + 1).text().startsWith("N")) {
    ...
    private static void enumerateElements(List<IBodyElement> elements, int period) {
        ...
        for (IBodyElement element : elements) {
            ...
            if (element instanceof XWPFParagraph) enumerateParagraph((XWPFParagraph) element, ordinal);
